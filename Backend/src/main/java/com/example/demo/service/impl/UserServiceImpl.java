...
public class UserServiceImpl implements UserService {
    ...
    public User createUser(UserRequest request) {
        ...
        if (request.getBanqueIds() != null) {
            banques = new HashSet<>(banqueRepository.findAllById(request.getBanqueIds()));
            user.setBanques(banques);
        }

        Set<Domaine> domaines = new HashSet<>();
        if (request.getDomaineIds() != null) {
            domaines = new HashSet<>(domaineRepository.findAllById(request.getDomaineIds()));
        ...
        if (request.getEtatIds() != null) {
            Set<Etat> etats = new HashSet<>(etatRepository.findAllById(request.getEtatIds()));

            for (Etat e : etats) {

                boolean validDomaine = domaines.contains(e.getDomaine());

                boolean validBanque = banques.stream()
                        .anyMatch(b -> b.getEtats().contains(e));

                if (!validDomaine || !validBanque) {
                    throw new IllegalArgumentException("Etat invalide pour cet utilisateur");
                }
            }
    ...
    public List<User> findByBanqueId(Long banqueId) {
        return userRepository.findAll().stream()
                .filter(u -> u.getBanques().stream().anyMatch(b -> b.getId().equals(banqueId)))
                .toList();
    }
